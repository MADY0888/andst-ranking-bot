...
CATEGORY_KEYWORDS = {
    "ワンピース": ["ワンピ", "OP", "ジャンスカ"],
    "トップス": ["トップス", "ブラウス", "シャツ", "Tシャツ", "カットソー", "ニット", "カーデ", "ビスチェ", "ベスト"],
    "ボトムス": ["パンツ", "デニム", "スカート", "ショートパンツ"],
    "アウター": ["ジャケット", "コート", "ブルゾン", "パーカー"],
    "シューズ": ["サンダル", "パンプス", "ブーツ", "スニーカー", "シューズ", "ミュール"],
    "バッグ": ["バッグ", "トート", "ショルダー", "ポーチ"],
    "アクセサリー": ["ピアス", "ネックレス", "リング", "イヤリング", "ヘア", "ベルト"],
}

BRAND_KEYWORDS = [
    "GRL", "グレイル", "田中みな実", "齋藤飛鳥", "ちぃぽぽ", "藤井サチ", "近藤千尋", "高橋愛",
    "U.S. POLO ASSN.", "Champion", "FILA", "KANGOL"
]
...
def load_slack_targets():
    ...
    if targets_json:
        ...
        for index, target in enumerate(raw_targets, start=1):
            ...
            if isinstance(channel_ids, str):
                channel_ids = [channel_id.strip() for channel_id in channel_ids.split(",") if channel_id.strip()]

            channel_ids = [str(channel_id).strip() for channel_id in channel_ids if str(channel_id).strip()]
    ...
    if SLACK_BOT_TOKEN and SLACK_CHANNEL_IDS:
        return [{"name": "default", "bot_token": SLACK_BOT_TOKEN, "channel_ids": SLACK_CHANNEL_IDS}]
...
def extract_brand(name):
    ...
    if bracket_match:
        return bracket_match.group(1)[:20]
    return "GRL"


def enrich_item(item):
    item["category"] = extract_category(item["name"])
...
def fetch_product_image(product_url):
    try:
        res = requests.get(product_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=20)
...
def fetch_grl():
    url = "https://www.grail.bz/disp/ranking/001001004/"
    ...
    for link in soup.select("a[href*='/item/']"):
        ...
        item = enrich_item({
            ...
            "price": price,
            "url": href,
            "image_url": fetch_product_image(href)
        })
        items.append(item)
...
def save_csv(items):
    ...
    with open(CSV_FILE, "a", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["date", "rank", "name", "price", "url", "image_url", "category", "brand"])
        for item in items:
            writer.writerow([
                today(), item["rank"], item["name"], item["price"], item["url"], item.get("image_url", ""),
                item.get("category") or extract_category(item["name"]), item.get("brand") or extract_brand(item["name"])
            ])
    ...
    with open(CSV_FILE, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            try:
                ...
                row["brand"] = row.get("brand") or extract_brand(row.get("name", ""))
...
def analyze_ranking(current, previous):
    ...
    for item in current:
        ...
        if old is None:
            ...
            if diff > 0:
                rising_items.append({**item, "old_rank": old["rank"], "rank_diff": diff})
...
def count_price_ranges(items):
    counter = Counter({"〜999円": 0, "1,000〜1,999円": 0, "2,000〜2,999円": 0, "3,000円〜": 0})
    for item in items:
        counter[price_range_name(item["price"])] += 1
    return counter
...
def auto_analysis(items, new_items, rising_items):
    ...
    top_brand = Counter(item.get("brand") or extract_brand(item["name"]) for item in items).most_common(1)[0][0]
    return "\n".join([
        f"上位は「{top_names}」が中心です。",
        price_analysis_text(items),
        f"カテゴリは「{top_category}」、ブランド/軸は「{top_brand}」が目立ちます。",
...
def analyze_period(rows):
    product_map = defaultdict(lambda: {
        "name": "", "url": "", "image_url": "", "category": "その他", "brand": "GRL", "price": 0,
        "appearances": 0, "rank_total": 0, "best_rank": 999, "first_rank": None, "last_rank": None,
        "first_date": None, "last_date": None, "rank1_count": 0,
    })
    ...
    for row in sorted(rows, key=lambda r: (r["date_obj"], r["rank"])):
        ...
        item.update({
            "name": row["name"], "url": row["url"], "image_url": row.get("image_url", ""),
            "category": row.get("category") or extract_category(row["name"]),
            "brand": row.get("brand") or extract_brand(row["name"]), "price": row["price"]
        })
        item["appearances"] += 1
    ...
    rising = sorted([p for p in products if p["rank_change"] > 0], key=lambda x: x["rank_change"], reverse=True)[:10]
    ...
    return {
        "days": len(dates), "product_count": len(products), "new_count": new_count,
        "popularity": popularity, "rising": rising, "champions": champions,
        "category_counts": category_counts, "brand_counts": brand_counts, "price_counts": price_counts,
        "top_categories": category_counts.most_common(5), "top_brands": brand_counts.most_common(5),
...
def draw_bar_chart(draw, title, data, x, y, w, h, font_title, font_small):
    draw.rounded_rectangle((x, y, x + w, y + h), radius=18, fill="#fff7fb", outline="#eeeeee")
    ...
    for label, value in items:
        draw.text((x + 20, bar_y), str(label)[:10], fill="#111111", font=font_small)
        bar_w = int(bar_max_w * (value / max_value)) if max_value else 0
        draw.rounded_rectangle((bar_x, bar_y + 3, bar_x + bar_w, bar_y + 21), radius=8, fill="#f7dce8")
...
def create_daily_report_image(items, analysis, new_items, rising_items):
    ...
    draw.rectangle((0, 0, width, 110), fill="#f7dce8")
    draw.text((40, 28), f"GRLランキング画像レポート / {today()}", fill="#111111", font=font_title)
    draw.text((930, 40), "自動分析レポート", fill="#333333", font=font_subtitle)
    ...
    for idx, item in enumerate(items[:10]):
        ...
        draw.rounded_rectangle((x, y, x + card_w, y + card_h), radius=18, fill="#ffffff", outline="#dddddd", width=2)
        draw.rounded_rectangle((x + 12, y + 12, x + 72, y + 52), radius=10, fill="#d99a2b")
        draw.text((x + 22, y + 18), f'{item["rank"]}位', fill="white", font=font_small)
        product_img = download_image(item["image_url"]) if item.get("image_url") else None
        if product_img:
        ...
        draw_text_wrap(draw, item["name"], (x + 14, y + 230), font_small, "#111111", card_w - 28, 23, max_lines=3)
        price_text = f'¥{item["price"]:,}' if item["price"] else "価格不明"
        draw.text((x + 14, y + 300), price_text, fill="#111111", font=font_body)
    ...
    draw.rounded_rectangle((box_x, box_y, box_x + box_w, box_y + box_h), radius=18, fill="#fff7fb", outline="#eeeeee")
    text_y = box_y + 30
    for line in analysis.split("\n"):
        if line.strip():
            text_y = draw_text_wrap(draw, "・" + line.strip(), (box_x + 25, text_y), font_body, "#111111", box_w - 50, 34)
            text_y += 8
    ...
    if rising_items:
        for item in rising_items[:5]:
            draw_text_wrap(draw, f'↑{item["rank_diff"]}：{item["old_rank"]}位 → {item["rank"]}位 {item["name"]}', (620, ry), font_small, "#111111", 520, 28, max_lines=1)
            ry += 32
    else:
    ...
    if new_items:
        for item in new_items[:5]:
            draw_text_wrap(draw, f'NEW {item["rank"]}位：{item["name"]}', (620, ny), font_small, "#111111", 520, 28, max_lines=1)
            ny += 32
    else:
    ...
    brand_counts = Counter(item.get("brand") or extract_brand(item["name"]) for item in items)
    draw_bar_chart(draw, "カテゴリ別TOP10構成", category_counts, 40, chart_y, 540, 250, font_subtitle, font_small)
    draw_bar_chart(draw, "ブランド/軸別TOP10構成", brand_counts, 620, chart_y, 540, 250, font_subtitle, font_small)
    ...
    draw.text((40, 2045), "CSVデータはGitHubに自動保存 / Google Sheetsは設定時のみ同期", fill="#555555", font=font_small)
    draw.text((850, 2045), "次回：明日 9:10 自動投稿", fill="#555555", font=font_small)
...
def create_period_report_image(report_type, period_title, summary):
    ...
    draw.rectangle((0, 0, width, 110), fill="#f7dce8")
    draw.text((40, 28), f"GRLランキング{report_label}レポート", fill="#111111", font=font_title)
    draw.text((760, 40), today(), fill="#333333", font=font_subtitle)
    draw.text((40, 145), period_title, fill="#111111", font=font_subtitle)

    stat_y = 210
    stats = [("集計日数", f"{summary['days']}日"), ("登場商品数", f"{summary['product_count']}商品"), ("初日新規数", f"{summary['new_count']}商品")]
    for idx, (label, value) in enumerate(stats):
        x = 40 + idx * 370
        draw.rounded_rectangle((x, stat_y, x + 330, stat_y + 110), radius=18, fill="#fff7fb", outline="#eeeeee")
    ...
    draw_bar_chart(draw, "ブランド/軸別トレンド", summary["brand_counts"], 620, 1400, 540, 240, font_subtitle, font_small)
    draw.rounded_rectangle((40, 1690, width - 40, 1750), radius=16, fill="#fff7fb", outline="#eeeeee")
...
def make_daily_link_text(items):
    lines = [f"📊 *GRLランキング画像レポート / {today()}*", "", "🔗 *商品リンク TOP10*"]
    for item in items[:10]:
        price_text = f'¥{item["price"]:,}' if item["price"] else "価格不明"
        lines.append(f'{item["rank"]}位：<{item["url"]}|{item["name"]}> {price_text}')
    return "\n".join(lines)


def make_daily_blocks(items, new_items, rising_items):
    category_top = Counter(item.get("category") or extract_category(item["name"]) for item in items).most_common(1)[0]
    brand_top = Counter(item.get("brand") or extract_brand(item["name"]) for item in items).most_common(1)[0]
    top_lines = "\n".join([f"*{item['rank']}位* <{item['url']}|{item['name'][:38]}>" for item in items[:5]])
    return [
        {"type": "header", "text": {"type": "plain_text", "text": f"GRLランキング TOP10 / {today()}", "emoji": True}},
        {"type": "section", "text": {"type": "mrkdwn", "text": top_lines}},
        {"type": "section", "fields": [
            {"type": "mrkdwn", "text": f"*新規*\n{len(new_items)}件"},
            {"type": "mrkdwn", "text": f"*急上昇*\n{len(rising_items)}件"},
            {"type": "mrkdwn", "text": f"*カテゴリ*\n{category_top[0]}"},
            {"type": "mrkdwn", "text": f"*ブランド/軸*\n{brand_top[0]}"},
        ]},
        {"type": "context", "elements": [{"type": "mrkdwn", "text": "画像レポートにはTOP10、グラフ、カテゴリ/ブランド分析を含みます。"}]},
    ]


def make_period_link_text(report_type, period_title, summary):
    label = "週次" if report_type == "weekly" else "月次"
    lines = [f"📊 *GRLランキング{label}レポート / {today()}*", period_title, "", f"集計日数：{summary['days']}日 / 登場商品数：{summary['product_count']}商品", "", "🏆 *人気継続 TOP10*"]
    for idx, item in enumerate(summary["popularity"][:10], start=1):
        lines.append(f"{idx}位：<{item['url']}|{item['name']}> 登場{item['appearances']}回 / 平均{item['avg_rank']:.1f}位")
    if summary["rising"]:
        lines += ["", "🔥 *急上昇 TOP5*"]
        for idx, item in enumerate(summary["rising"][:5], start=1):
            lines.append(f"{idx}位：↑{item['rank_change']} {item['first_rank']}位→{item['last_rank']}位 <{item['url']}|{item['name']}>")
    return "\n".join(lines)


def make_period_blocks(report_type, period_title, summary):
    label = "週次" if report_type == "weekly" else "月次"
    popular = "\n".join([f"*{idx}位* <{item['url']}|{item['name'][:38]}> 登場{item['appearances']}回" for idx, item in enumerate(summary["popularity"][:5], start=1)]) or "データなし"
    top_category = summary["top_categories"][0][0] if summary["top_categories"] else "なし"
    top_brand = summary["top_brands"][0][0] if summary["top_brands"] else "なし"
    return [
        {"type": "header", "text": {"type": "plain_text", "text": f"GRLランキング{label}レポート", "emoji": True}},
        {"type": "section", "text": {"type": "mrkdwn", "text": period_title}},
        {"type": "section", "fields": [
            {"type": "mrkdwn", "text": f"*集計日数*\n{summary['days']}日"},
            {"type": "mrkdwn", "text": f"*登場商品*\n{summary['product_count']}商品"},
            {"type": "mrkdwn", "text": f"*カテゴリ1位*\n{top_category}"},
            {"type": "mrkdwn", "text": f"*ブランド/軸1位*\n{top_brand}"},
        ]},
        {"type": "section", "text": {"type": "mrkdwn", "text": "🏆 *人気継続 TOP5*\n" + popular}},
    ]


def post_to_slack(image_path, comment, title, blocks=None):
...
def sync_daily_to_google_sheets(items):
    ...
    if sheet is None:
        return
    ws = get_or_create_worksheet(sheet, "daily_ranking")
    if not ws.get_all_values():
        ws.append_row(["date", "rank", "name", "price", "url", "image_url", "category", "brand"], value_input_option="USER_ENTERED")
    rows = [[today(), item["rank"], item["name"], item["price"], item["url"], item.get("image_url", ""), item.get("category", ""), item.get("brand", "")] for item in items]
...
def sync_period_to_google_sheets(report_type, period_title, summary):
    ...
    if sheet is None:
        return
    ws = get_or_create_worksheet(sheet, "period_reports")
    if not ws.get_all_values():
        ws.append_row(["created_at", "report_type", "period", "days", "product_count", "top_category", "top_brand", "top_item"], value_input_option="USER_ENTERED")
    ...
    ws.append_row([today(), report_type, period_title, summary["days"], summary["product_count"], top_category, top_brand, top_item], value_input_option="USER_ENTERED")
...
def run_daily():
    ...
    post_to_slack(image_path, make_daily_link_text(current), f"GRLランキング画像レポート {today()}", make_daily_blocks(current, new_items, rising_items))
...
def run_period(report_type):
    ...
    label = "週次" if report_type == "weekly" else "月次"
    post_to_slack(image_path, make_period_link_text(report_type, period_title, summary), f"GRLランキング{label}レポート {today()}", make_period_blocks(report_type, period_title, summary))
    sync_period_to_google_sheets(report_type, period_title, summary)


def main():
    parser = argparse.ArgumentParser(description="GRL ranking Slack report bot")
    parser.add_argument("--report-type", choices=["daily", "weekly", "monthly"], default="daily", help="投稿するレポート種別")
